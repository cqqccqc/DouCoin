...
class Trade extends CI_Controller {
	...
	private function _transaction($cid)
	{
		...
		$bp=$query1->row_array()['price'];
		$bn=$query1->row_array()['num'];
		$boid=$query1->row_array()['oid'];
		$buid=$query1->row_array()['uid'];
		$brmb=$bp*$bn;

		$query2=$this->db->query($sql2);
		$sp=$query2->row_array()['price'];
		$sn=$query2->row_array()['num'];
		$soid=$query2->row_array()['oid'];
		$suid=$query2->row_array()['uid'];
		$srmb=$sp*$sn;
		...
		if ($bp>=$sp)
		{
			...
			{	//order deal
				...
				$bormb=$this->db->get()->row_array()['num'];
				$this->db->select('num')->from('property')->where(array('uid'=>$suid,'cid'=>17));
				$sormb=$this->db->get()->row_array()['num'];
				$brmb-=$bormb;
				$srmb+=$sormb;
